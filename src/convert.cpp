...
void do_conversion(string filename, string(*func)(string, string, int, int, int));
string convert_from_sword(string key, string version, int bookNo, int chapter, int verse);
...
void sql_setup_db() {
	string Bible_sql("CREATE TABLE IF NOT EXISTS \"Bible\" (\"Book\" INT,\"Chapter\" INT,\"Verse\" INT,\"Scripture\" TEXT); CREATE UNIQUE INDEX \"bible_key\" ON \"Bible\" (\"Book\" ASC, \"Chapter\" ASC, \"Verse\" ASC);");
	string Details_sql("CREATE TABLE IF NOT EXISTS \"Details\" (\"Description\" NVARCHAR(255),\"Abbreviation\" NVARCHAR(50),\"Comments\" TEXT,\"Version\" TEXT, \"VersionDate\" DATETIME, \"PublishDate\" DATETIME,\"RightToLeft\" BOOL,\"OT\" BOOL,\"NT\" BOOL,\"Strong\" BOOL);");
...
void sqlite_write(string _scripture, int _book, int _chapter, int _verse) {
	const char *sql_ = "INSERT INTO Bible (Book, Chapter, Verse, Scripture) VALUES (?, ?, ?, ?)";
	sqlite3_stmt *stmt;
	const char *test;
	...
	if (rc == SQLITE_OK) {
		...
		sqlite3_bind_text(stmt, 4, _scripture.c_str(), strlen(_scripture.c_str()), 0);
...
string get_sql_verse(int book, int chapter, int verse) {
	sqlite3_stmt *stmt;
	const char *test;
	const char *sql = "SELECT Scripture FROM Bible WHERE Book = ? AND Chapter = ? AND Verse = ?;";
...
int max_verse(int book, int chapter) {
	int res_ = 0;
	const char* sql_ = "SELECT Max(Verse) FROM Bible WHERE Book = ? AND Chapter = ?;";
	const char* test;
	sqlite3_stmt *stmt;
...
string get_verse(const char* _ver, const char* _key) {
	sword::SWMgr manager(new sword::MarkupFilterMgr(sword::FMT_PLAIN, sword::ENC_UTF8));
